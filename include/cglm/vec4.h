...
#ifndef __INCLUDE_CGLM_VEC4_H__
#define __INCLUDE_CGLM_VEC4_H__
...
#if defined(__wasm__) && defined(__wasm_simd128__)
    glmm_store(dest,
               wasm_f32x4_add(glmm_load(dest), glmm_max(glmm_load(a), glmm_load(b))));
#elif defined(__SSE__) || defined(__SSE2__)
    glmm_store(dest, _mm_add_ps(glmm_load(dest), glmm_max(glmm_load(a), glmm_load(b))));
#elif defined(CGLM_NEON_FP)
    glmm_store(dest, vaddq_f32(glmm_load(dest), glmm_max(glmm_load(a), glmm_load(b))));
#else
...
#if defined(__wasm__) && defined(__wasm_simd128__)
    glmm_store(dest,
               wasm_f32x4_add(glmm_load(dest), glmm_min(glmm_load(a), glmm_load(b))));
#elif defined(__SSE__) || defined(__SSE2__)
    glmm_store(dest, _mm_add_ps(glmm_load(dest), glmm_min(glmm_load(a), glmm_load(b))));
#elif defined(CGLM_NEON_FP)
    glmm_store(dest, vaddq_f32(glmm_load(dest), glmm_min(glmm_load(a), glmm_load(b))));
#else
 ...
 */
CGLM_INLINE
void glm_vec4_subsub(vec4 a, vec4 b, vec4 dest) {
#if defined(__wasm__) && defined(__wasm_simd128__)
    glmm_store(dest, wasm_f32x4_sub(glmm_load(dest),
                                    wasm_f32x4_sub(glmm_load(a), glmm_load(b))));
#elif defined(__SSE__) || defined(__SSE2__)
    glmm_store(dest, _mm_sub_ps(glmm_load(dest), _mm_sub_ps(glmm_load(a), glmm_load(b))));
#elif defined(CGLM_NEON_FP)
    vst1q_f32(dest, vsubq_f32(vld1q_f32(dest), vsubq_f32(vld1q_f32(a), vld1q_f32(b))));
#else
    dest[0] -= a[0] - b[0];
    dest[1] -= a[1] - b[1];
    dest[2] -= a[2] - b[2];
 ...
 */
CGLM_INLINE
void glm_vec4_addsub(vec4 a, vec4 b, vec4 dest) {
#if defined(__wasm__) && defined(__wasm_simd128__)
    glmm_store(dest, wasm_f32x4_sub(glmm_load(dest),
                                    wasm_f32x4_add(glmm_load(a), glmm_load(b))));
#elif defined(__SSE__) || defined(__SSE2__)
    glmm_store(dest, _mm_sub_ps(glmm_load(dest), _mm_add_ps(glmm_load(a), glmm_load(b))));
#elif defined(CGLM_NEON_FP)
    vst1q_f32(dest, vsubq_f32(vld1q_f32(dest), vaddq_f32(vld1q_f32(a), vld1q_f32(b))));
#else
    dest[0] -= a[0] + b[0];
    dest[1] -= a[1] + b[1];
    dest[2] -= a[2] + b[2];
 ...
 */
CGLM_INLINE
void glm_vec4_mulsub(vec4 a, vec4 b, vec4 dest) {
#if defined(CGLM_SIMD)
 ...
 */
CGLM_INLINE
void glm_vec4_mulsubs(vec4 a, float s, vec4 dest) {
#if defined(CGLM_SIMD)
 ...
 */
CGLM_INLINE
void glm_vec4_maxsub(vec4 a, vec4 b, vec4 dest) {
#if defined(__wasm__) && defined(__wasm_simd128__)
    glmm_store(dest,
               wasm_f32x4_sub(glmm_load(dest), glmm_max(glmm_load(a), glmm_load(b))));
#elif defined(__SSE__) || defined(__SSE2__)
    glmm_store(dest, _mm_sub_ps(glmm_load(dest), glmm_max(glmm_load(a), glmm_load(b))));
#elif defined(CGLM_NEON_FP)
    glmm_store(dest, vsubq_f32(glmm_load(dest), glmm_max(glmm_load(a), glmm_load(b))));
#else
    dest[0] -= glm_max(a[0], b[0]);
    dest[1] -= glm_max(a[1], b[1]);
    dest[2] -= glm_max(a[2], b[2]);
 ...
 */
CGLM_INLINE
void glm_vec4_minsub(vec4 a, vec4 b, vec4 dest) {
#if defined(__wasm__) && defined(__wasm_simd128__)
    glmm_store(dest,
               wasm_f32x4_sub(glmm_load(dest), glmm_min(glmm_load(a), glmm_load(b))));
#elif defined(__SSE__) || defined(__SSE2__)
    glmm_store(dest, _mm_sub_ps(glmm_load(dest), glmm_min(glmm_load(a), glmm_load(b))));
#elif defined(CGLM_NEON_FP)
    glmm_store(dest, vsubq_f32(vld1q_f32(dest), glmm_min(glmm_load(a), glmm_load(b))));
#else
    dest[0] -= glm_min(a[0], b[0]);
    dest[1] -= glm_min(a[1], b[1]);
    dest[2] -= glm_min(a[2], b[2]);
...
#else
    return sqrtf(glm_pow2(a[0] - b[0]) + glm_pow2(a[1] - b[1]) + glm_pow2(a[2] - b[2]) +
                 glm_pow2(a[3] - b[3]));
#endif
}
...
#elif defined(__SSE__) || defined(__SSE2__)
    glmm_store(
        v, glmm_min(glmm_max(glmm_load(v), _mm_set1_ps(minVal)), _mm_set1_ps(maxVal)));
#elif defined(CGLM_NEON_FP)
    glmm_store(
        v, glmm_min(glmm_max(vld1q_f32(v), vdupq_n_f32(minVal)), vdupq_n_f32(maxVal)));
#else
 ...
 */
CGLM_INLINE
void glm_vec4_make(const float *__restrict src, vec4 dest) {
    dest[0] = src[0];
    dest[1] = src[1];
    dest[2] = src[2];
    dest[3] = src[3];
 ...
 */
CGLM_INLINE
bool glm_vec4_refract(vec4 v, vec4 n, float eta, vec4 dest) {
    float ndi, eni, k;
